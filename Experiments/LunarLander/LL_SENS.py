...
PRINT_TREE = False
SENS_VERS = (
    "V0"  # "V0" or "V1", old or new version of sensitivity, see notes_pplot_sens.docx
)
...
n_tree_evaluation_eps = (
    100  # how many reward-evaluation episodes for tree in sensitivity
)
...
class LL_Sensitivity(e_utils.SensitivityStrategy):
    ...
    def sensitivity_analysis(self, inode=0, setzero=None):
        ...
        with open(
            "../../Paper_Results/OPCTs/LunarLander-v2_ITER_depth_2.pkl", "rb"
        ) as input:
...
def main_part_sensitivity():
    if RELOAD:
        ...
        analyze_pickles()
        df_res_dict = e_utils.generate_sens_plots(
            prefix="LL_",
            ylim=ylim,
        ...
        if SINGLE_NODE:
            ...
            df_res_dict = e_utils.generate_multi_sens_pkls(
                ll_ses,
                inodes,
                setzeros,
                prefix="LL_",
...
def main_part_attract_plot():
    ...
    env = le.LunarLanderEnvironment().get_environment()
    std = np.array(
        [0.020306, 0.142200, 0.008996, 0.204207, 0.020306, 0.142200, 0.008996, 0.204207]
    )
    with open(
        "../../Paper_Results/OPCTs/LunarLander-v2_ITER_depth_2.pkl", "rb"
    ) as input:
...
def main_part_distance_plot():
    ...
    with open(
        "../../Paper_Results_v2/OPCTs/LunarLander-v2_ITER_depth_2.pkl", "rb"
    ) as input:
        best_tree = pickle.load(input)
    tree_reward_mean, tree_reward_std, tree_samples = ll_evs.eval_opct(
        env, best_tree, n_tree_plane_eps, std
