...
class CP_Sensitivity(e_utils.SensitivityStrategy):
    ...
    def sensitivity_analysis(self, inode=0, setzero=None):
        ...
        with open(
            "../../Paper_Results/OPCTs/CartPole-v1_ITER_depth_1.pkl", "rb"
        ) as input:
...
def main_part_sensitivity():
    if RELOAD:
        ...
        df_res_dict = e_utils.generate_sens_plots(
            prefix="CP_", ylim=ylim, last_levels=["velocity"]
        )
    else:
        cp_evs = CP_EvalStrategy(reward_solved, n_tree_evaluation_eps)
        ...
        df_res_dict = e_utils.generate_sens_pkls(
            cp_ses, inodes, setzeros, prefix="CP_", ylim=ylim, last_levels=["velocity"]
        )
