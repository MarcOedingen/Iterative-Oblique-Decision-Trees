...
if __name__ == "__main__":
    ...
    td3_mcc = lo.TD3_Oracle().load_model(le.MountainCarContinuousEnvironment().get_environmentShort())
    ...
    for k in range(numb_runs):
        ...
        for i in range(max_depth):
            ...
            for j in range(numb_trees):
                ...
                mcc_cart.fit(mcc_samples.iloc[:, :-1].values, mcc_samples.iloc[:, -1].values)
    ...
    p_utils.save_results("MountainCarContinuous-v0", rewards_cart, rewards_oracle, method="CART")
    p_utils.save_best_cart(
        best_carts, rewards_cart, reward_threshold, env_name="MountainCarContinuous-v0", method="CART"
    )
    p_utils.save_timings_samples(
        time_run, time_cart, samples_cart, env_name="MountainCarContinuous-v0", method="CART"
    )
