...
class MCC_EvalStrategy(e_utils.EvalStrategy):
    ...
    def eval_opct(self, env, opct, eps, sig):
        ...
        for e in range(eps):
            ...
            while not done:
                action = opct.predict(s.reshape(1, -1))[0][0]
...
def plot_attract_mcc(inode, opct, samples, pngfile, ttype):
    """
    ...
    e_utils.append_dist_cols(inode, opct, samples, observations, ttype="regr")
...
def plot_all_attract_mcc(opct, samples, ttype="regr"):
    ...
    for inode in range(opct.trees[0].size):
        ...
        if node.left != -1:  # if it is not a leaf node:
            ...
            samples = plot_attract_mcc(inode, opct, samples, pngfile, ttype)
