...
from Paper_Results_v2 import utils as p_utils
...
if __name__ == "__main__":
    ...
    print(
        "----------------------Starting experiment for CartPoleSwingUp-v1 Iterative samples with OPCT----------------------"
    )
    for i in range(numb_runs):
        time_start_run = time.time()
        base_samples, rewards = o_utils.evaluate_oracle_cpsu_steps(
    ...
    p_utils.save_results(
        "CartPoleSwingUp-v1", rewards_opct, rewards_oracle, method="ITER"
    )
    ...
    p_utils.save_timings_samples(
        time_run, time_opct, samples_opct, env_name="CartPoleSwingUp-v1", method="ITER"
    )
