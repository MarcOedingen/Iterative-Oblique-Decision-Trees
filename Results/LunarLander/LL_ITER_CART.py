...
if __name__ == "__main__":
    ...
    print(
        "----------------------Starting experiment for LunarLander-v2 Iterative samples with CART----------------------"
    )
    for i in range(numb_runs):
        time_start_run = time.time()
        base_samples, rewards = o_utils.evaluate_oracle_ll_steps(
        ...
        for j in range(max_depth):
            ...
            for k in range(num_iter + 1):
                ...
                (cart, samples, reward, std) = p_utils.get_best_class_cart(
                    num_trees=num_trees,
                ...
                if reward > best_reward:
                    ...
                    best_samples = copy.deepcopy(samples)
                    best_reward = reward
                    best_std = std
            ...
            time_cart[j, i] = np.sum(cart_times)
    ...
    p_utils.save_results("LunarLander-v2", rewards_cart, rewards_oracle, method="ITER_CART")
    ...
    p_utils.save_timings_samples(
        time_run, time_cart, samples_cart, env_name="LunarLander-v2", method="ITER_CART"
    )
